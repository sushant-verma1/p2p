...
- **CGNAT.** Most Indian home broadband and all mobile data put the node behind carrier-grade NAT, where inbound connections are impossible. With no hole punching in scope, real-world testing requires a VPS, a LAN, or a port-forwarded connection. This is the single largest practical limitation, but since M9d it binds one side only: the *requester* dials, so a peer behind CGNAT can connect out to an invite's owner and hold a session. What CGNAT still blocks is being that owner — a node nobody can reach cannot hand out a usable invite, and both ends behind it cannot meet at all.
