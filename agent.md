...
- **Trusting a test harness because it came back green.** A harness needs the same scrutiny as the code it measures. At M12c, `netem/tail.py` counted a retry only when a failure event appeared. A restarted node redials from the reconnect loop, which retries without raising one. So samples that dialled twice were reported as clean. The instrument could not contradict the property it was measuring. This is the same shape as `foreign_keys` at M6: the pragma test passed with the line deleted, because the bundled SQLite defaults it on. It is also the same shape as the nonces at M4: they are redundant with the channel binding, so nothing fails without them. A green measurement from a blind instrument looks exactly like a real one. Before trusting a number, check that the instrument can report the opposite. Make it fail once on purpose, or confirm that it counts the thing from a second source.
